...
> **This project uses Vite+.** Import test utilities from `vite-plus/test` instead of `vitest`. Import browser context from `vite-plus/test/browser/context` instead of `@vitest/browser/context`. Run tests with `vp test`.
